...
%   \xdef\opa{0.11}
    ...
    \draw (e1v) ++ (0,1) to[rmeter, t=A] ++(1.5,0) coordinate (a);

    \draw (a) to[R,l^=$R_3\ 1.5\mathrm{k}$] ++ (1,0) -| coordinate (in) (fet.D);

    \draw (in) to[short,*-*] ++ (0.5,0) node[right] {Осциллограф};

    \draw ($(fet.G)!0.5!(G)$) to[short,*-] ++ (0,0.8)
        to[C, l^=$C_1$] ++ (0.8,0) to[short,-*] ++ (0.5,0) node[right,align=left] {Генератор\\сигналов};
